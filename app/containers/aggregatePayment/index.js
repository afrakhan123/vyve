...
const AggregatePaymentScreen = ({ onComplete = () => { }, onCancel = () => { } }) => {
  const api = useApi()
  ...
  const cost = useNavigationParam('costRemaining');
  const totalKgToOffset = useNavigationParam('totalKgToOffset');
  const paymentIdArray = useNavigationParam('paymentIdArray');
  const view = useNavigationParam('view');
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.checkoutContainer}>
        ...
        <View style={styles.row}>
          <View style={{ flex: 2 }}>
            <Text style={styles.paymentTitle}>Emissions {view}</Text>
            <Text>{totalKgToOffset}kg CO2e</Text>
          </View>
          <View style={{ flex: 1 }}>
            <Text style={{ textAlign: 'right' }}>Total</Text>
            <Text style={{ textAlign: 'right', fontWeight: 'bold' }}>£{cost}</Text>
          </View>
        </View>
        <Border />
        <PaymentSummary />
...
export default AggregatePaymentScreen